...
Usage:
  peaks_filter.R [options] <broadPeak_input> <csv_input> <threshold_value> <output>
...
peaks_to_keep = peaks_readcount  %>% 
  dplyr::filter(nbreads>=as.numeric(arguments$threshold_value)) %>%                          # threshold application, suppress peaks that don't pass nbreads >= 10
